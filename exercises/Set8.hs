...
paint (Picture pat) (Shape s) (Picture base) = Picture f
  where f coord | s coord   = pat coord
                | otherwise = base coord
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where g (Coord x y) =
            let Color r0 g0 b0 = f (Coord x y)
                Color r1 g1 b1 = f (Coord (x-1) y)
                Color r2 g2 b2 = f (Coord (x+1) y)
                Color r3 g3 b3 = f (Coord x (y-1))
                Color r4 g4 b4 = f (Coord x (y+1))
            in Color (div (r0+r1+r2+r3+r4) 5)
                     (div (g0+g1+g2+g3+g4) 5)
                     (div (b0+b1+b2+b3+b4) 5)
------------------------------------------------------------------------------
...
instance Transform BlurMany where
  apply (BlurMany n) picture
    | n <= 0    = picture
    | otherwise = apply (BlurMany (n-1)) (apply Blur picture)
------------------------------------------------------------------------------
