...
visit maze place = do
  visited <- get
  when (place `notElem` visited) $ do
    modify (place:)
    let neighbours = maybe [] id (lookup place maze)
...
path maze place1 place2 = place2 `elem` reachable
  where reachable = execState (visit maze place1) []
