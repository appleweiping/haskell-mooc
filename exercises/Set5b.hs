...
allValues condition (Node x l r) =
  condition x && allValues condition l && allValues condition r
...
isOrdered (Node x l r) =
  allValues (< x) l && allValues (> x) r && isOrdered l && isOrdered r
...
set (StepL:path) val (Node x l r) = Node x (set path val l) r
set (StepR:path) val (Node x l r) = Node x l (set path val r)
