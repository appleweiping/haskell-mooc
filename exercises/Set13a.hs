...
checkCapitals (for,sur)
  | startsCapital for && startsCapital sur = Just (for, sur)
  | otherwise                              = Nothing
  where startsCapital (c:_) = isUpper c
        startsCapital []    = False
...
selectSum xs is = do
  ...
  return (sum vals)
  where safeIndex ys i
          | i >= 0 && i < length ys = Just (ys !! i)
          | otherwise               = Nothing
...
countAndLog pred (x:xs)
  | pred x    = msg (show x) >> fmap (+1) (countAndLog pred xs)
...
balance accountName = BankOp (\bank@(Bank accounts) -> (Map.findWithDefault 0 accountName accounts, bank))
...
rob from to =
  balance from
  +>
  \amount -> withdrawOp from amount
  +>
  \taken -> depositOp to taken
...
count x = modify (bump x)
  where bump y [] = [(y,1)]
        bump y ((k,n):rest)
          | k == y    = (k, n+1) : rest
          | otherwise = (k,n) : bump y rest
