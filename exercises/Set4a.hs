...
longest :: Ord a => [[a]] -> [a]
longest = minimumBy (comparing (\xs -> (negate (length xs), head xs)))
...
freqs = foldr (\x -> Map.alter bump x) Map.empty
  where bump Nothing  = Just 1
...
transfer from to amount bank
  | Map.member from bank
    && Map.member to bank
    && amount >= 0
    && Map.findWithDefault 0 from bank >= amount =
