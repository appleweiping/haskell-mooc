...
rankCountry :: Country -> Int
rankCountry Finland     = 0
rankCountry Norway      = 1
rankCountry Switzerland = 2

instance Ord Country where
  compare a b = compare (rankCountry a) (rankCountry b)
  a <= b      = rankCountry a <= rankCountry b
  min a b     = if rankCountry a <= rankCountry b then a else b
  max a b     = if rankCountry a >= rankCountry b then a else b
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a _) = RationalNumber (signum a) 1
