...
prettyPrint n queens =
  concat [[cell i j | j <- [1..n]] ++ "\n" | i <- [1..n]]
  where cell i j | (i,j) `elem` queens = 'Q'
                 | otherwise           = '.'
...
danger c = any threatens
  where threatens q = sameRow c q || sameCol c q || sameDiag c q || sameAntidiag c q
...
backtrack [] = []
backtrack [_] = []
backtrack (_:next:rest) = nextCol next : rest
