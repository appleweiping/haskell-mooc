...
statements xs ys =
  concat (liftA2 (\x y -> [x ++ " is " ++ y, x ++ " is not " ++ y]) xs ys)
...
calculator op number = liftA2 (\f n -> f n) operation (readMaybe number)
...
validateAddress streetName streetNumber postCode =
  liftA3 Address checkedName checkedNumber checkedPost
  where checkedName   = check (length streetName <= 20) "Invalid street name" streetName
        ...
        checkedPost   = check (length postCode == 5 && all isDigit postCode) "Invalid postcode" postCode
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
  liftA2 (\p1 p2 -> [p1, p2])
    (liftA3 Person name1 age1 employed1)
    (liftA3 Person name2 age2 employed2)
...
boolOrInt s = asBool <|> asInt
  where asBool = case readMaybe s :: Maybe Bool of
          Just b  -> pure (Left b)
          Nothing -> invalid "Not a Bool"
        asInt = case readMaybe s :: Maybe Int of
          Just i  -> pure (Right i)
...
parseExpression s = case words s of
  [a, op, b] -> liftA3 build (parseOp op) (parseArg a) (parseArg b)
  ...
  where
    ...
    parseArg w = case readMaybe w :: Maybe Int of
      Just n  -> pure (Number n)
      Nothing -> case w of
        [c] | isAlpha c -> pure (Variable c)
        _ -> invalid ("Invalid number: " ++ w) *> invalid ("Invalid variable: " ++ w)
